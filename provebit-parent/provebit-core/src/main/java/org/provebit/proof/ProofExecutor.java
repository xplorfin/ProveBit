...
public class ProofExecutor {
	...
	public byte[] execute(ProofParser prg) {
		...
		while (executionIsUnfinished()) {
			...
			if (opname.startsWith("f_")) {
				if (functionFrames.size() > (STACK_FRAMES - 1))
					throw new RuntimeException(
							"proof executor stack frame past max");
			...
			case "op_compose_ext":
				...
				if (sub == -1)
					throw new ProgramDieException(
							"Byte subarray fail in op_compose");
	...
	private int helperIntBounds(BigInteger i, int min, int max) {
		if (i.compareTo(BigInteger.valueOf(max)) > 0
			...
			throw new RuntimeException("location: " + i
					+ " out of array bounds");
		return i.intValue();
	}
	...
	private byte[] mread(int location) {
		if (location < WORKING_LOCATION || location > MEM_SIZE)
			throw new MemoryCellException("location: " + location
					+ " out of bounds");
	...
	private String getString(String fname, int op, int arg) {
		try {
			...
			if (bytes == null)
				bytes = new byte[0]; // default memory
